...
func (a *App) writeDevinHookHelp() {
	writeHelp(a.stdout, []string{
		...
		"emit one receive instruction: waypost_recv when available, CLI otherwise.",
		"After a successful receive, PostCompaction marks an anti-repeat guard",
		"pending; the next PostToolUse, UserPromptSubmit, or SessionStart emits it",
		"because Devin drops PostCompaction additionalContext.",
